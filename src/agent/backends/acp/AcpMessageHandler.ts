...
import { asString, isObject } from '../../../utils/protocol';
import { deriveToolNameWithSource, isPlaceholderToolName } from '../../utils';
...
export class AcpMessageHandler {
    ...
    private appendTextChunk(text: string): void {
        if (!text) return;
        if (!this.bufferedText) { this.bufferedText = text; return; }
        if (text === this.bufferedText) return;
        if (text.startsWith(this.bufferedText)) { this.bufferedText = text; return; }
        if (this.bufferedText.startsWith(text)) return;
        if (this.bufferedText.endsWith(text)) return;
        if (text.endsWith(this.bufferedText)) { this.bufferedText = text; return; }
        const overlap = getSuffixPrefixOverlap(this.bufferedText, text);
        if (overlap > 0) { this.bufferedText += text.slice(overlap); return; }
    ...
    handleUpdate(update: unknown): void {
        ...
        if (updateType === ACP_SESSION_UPDATE_TYPES.agentMessageChunk) {
            ...
            if (text) this.appendTextChunk(text);
            return;
        }
        if (updateType === ACP_SESSION_UPDATE_TYPES.agentThoughtChunk) return;
        if (updateType === ACP_SESSION_UPDATE_TYPES.toolCall) { this.handleToolCall(update); return; }
        if (updateType === ACP_SESSION_UPDATE_TYPES.toolCallUpdate) { this.handleToolCallUpdate(update); return; }
        if (updateType === ACP_SESSION_UPDATE_TYPES.plan) {
            const items = normalizePlanEntries(update.entries);
            if (items.length > 0) this.onMessage({ type: 'plan', items });
        }
    }
    ...
    private handleToolCall(update: Record<string, unknown>): void {
        ...
        this.toolCalls.set(toolCallId, { name: derivedName.name, input });
        this.onMessage({ type: 'tool_call', id: toolCallId, name: derivedName.name, input, status });
    }
    ...
    private handleToolCallUpdate(update: Record<string, unknown>): void {
        ...
        if (update.rawInput !== undefined) {
            ...
            this.onMessage({ type: 'tool_call', id: toolCallId, name, input, status });
        } else if (existing && (status === 'in_progress' || status === 'pending')) {
            this.onMessage({ type: 'tool_call', id: toolCallId, name: existing.name, input: existing.input, status });
        }

        if (status === 'completed' || status === 'failed') {
            const result = update.rawOutput ?? update.content;
            this.onMessage({ type: 'tool_result', id: toolCallId, output: result, status: status === 'failed' ? 'failed' : 'completed' });
        }
    }

    private selectToolNameForUpdate(existingName: string | null, derivedName: DerivedToolName): string {
        if (!existingName) return derivedName.name;
        if (derivedName.source === 'title' || derivedName.source === 'raw_input_name' || derivedName.source === 'raw_input_tool') {
            return derivedName.name;
        }
        if (isPlaceholderToolName(existingName)) return derivedName.name;
        return existingName;
    }
}
