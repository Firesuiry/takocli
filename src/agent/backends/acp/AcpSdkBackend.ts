import type { AgentBackend, AgentMessage, AgentSessionConfig, PermissionRequest, PermissionResponse, PromptContent } from '../../types';
import { asString, isObject, withRetry } from '../../../utils/protocol';
...
export class AcpSdkBackend implements AgentBackend {
    ...
    private static readonly INIT_RETRY_OPTIONS = { maxAttempts: 3, minDelay: 1000, maxDelay: 5000 };
    ...
    async initialize(): Promise<void> {
        ...
        this.transport.registerRequestHandler('session/request_permission', async (params) => {
        ...
        const response = await withRetry(
            () => this.transport!.sendRequest('initialize', {
                ...
                clientInfo: { name: 'tako', version: VERSION }
            }),
            {
                ...AcpSdkBackend.INIT_RETRY_OPTIONS,
                onRetry: (_error, attempt, nextDelayMs) => {
                    log.debug(`[ACP] Initialize attempt ${attempt} failed, retrying in ${nextDelayMs}ms`);
                }
            }
        );
        ...
        log.debug(`[ACP] Initialized with protocol version ${response.protocolVersion}`);
    }

    async newSession(config: AgentSessionConfig): Promise<string> {
        if (!this.transport) throw new Error('ACP transport not initialized');

        const response = await withRetry(
            () => this.transport!.sendRequest('session/new', { cwd: config.cwd, mcpServers: config.mcpServers }),
            {
                ...AcpSdkBackend.INIT_RETRY_OPTIONS,
                onRetry: (_error, attempt, nextDelayMs) => {
                    log.debug(`[ACP] session/new attempt ${attempt} failed, retrying in ${nextDelayMs}ms`);
                }
            }
        );

        const sessionId = isObject(response) ? asString(response.sessionId) : null;
        if (!sessionId) throw new Error('Invalid session/new response from ACP agent');
    ...
    async loadSession(config: AgentSessionConfig & { sessionId: string }): Promise<string> {
        ...
        const response = await withRetry(
            () => this.transport!.sendRequest('session/load', {
                sessionId: config.sessionId, cwd: config.cwd, mcpServers: config.mcpServers
            }),
            {
                ...AcpSdkBackend.INIT_RETRY_OPTIONS,
                onRetry: (_error, attempt, nextDelayMs) => {
                    log.debug(`[ACP] session/load attempt ${attempt} failed, retrying in ${nextDelayMs}ms`);
                }
            }
        );
    ...
    async prompt(sessionId: string, content: PromptContent[], onUpdate: (msg: AgentMessage) => void): Promise<void> {
        ...
        this.activeSessionId = sessionId;
        await this.waitForSessionUpdateQuiet(AcpSdkBackend.PRE_PROMPT_UPDATE_QUIET_PERIOD_MS, AcpSdkBackend.PRE_PROMPT_UPDATE_DRAIN_TIMEOUT_MS);
        ...
        try {
            const response = await this.transport.sendRequest('session/prompt', { sessionId, prompt: content }, { timeoutMs: Infinity });
            stopReason = isObject(response) ? asString(response.stopReason) : null;
        } finally {
            await this.waitForSessionUpdateQuiet(AcpSdkBackend.UPDATE_QUIET_PERIOD_MS, AcpSdkBackend.UPDATE_DRAIN_TIMEOUT_MS);
            this.messageHandler?.flushText();
            try {
                if (stopReason) onUpdate({ type: 'turn_complete', stopReason });
            } finally {
    ...
    async respondToPermission(_sessionId: string, request: PermissionRequest, response: PermissionResponse): Promise<void> {
        ...
        if (!pending) { log.debug(`[ACP] No pending permission request for id ${request.id}`); return; }
    ...
    get processingMessage(): boolean { return this.isProcessingMessage; }
    getLastSessionUpdateAt(): number { return this.lastSessionUpdateAt; }

    async waitForResponseComplete(): Promise<void> {
        if (!this.isProcessingMessage) return;
        return new Promise<void>((resolve) => { this.responseCompleteResolvers.push(resolve); });
    }
    ...
    private handleSessionUpdate(params: unknown): void {
        ...
        this.lastSessionUpdateAt = Date.now();
        this.messageHandler?.handleUpdate(params.update);
    }

    private async waitForSessionUpdateQuiet(quietMs: number, timeoutMs: number): Promise<void> {
        if (quietMs <= 0 || timeoutMs <= 0) return;
        const deadline = Date.now() + timeoutMs;
        while (Date.now() < deadline) {
            const elapsedSinceUpdate = Date.now() - this.lastSessionUpdateAt;
            if (elapsedSinceUpdate >= quietMs) return;
            const waitMs = Math.max(1, Math.min(quietMs - elapsedSinceUpdate, deadline - Date.now()));
    ...
    private async handlePermissionRequest(params: unknown): Promise<unknown> {
        ...
        const request: PermissionRequest = { id: toolCallId, sessionId, toolCallId, title, kind, rawInput, rawOutput, options };

        if (this.permissionHandler) {
            this.permissionHandler(request);
        } else {
            log.debug('[ACP] No permission handler registered; cancelling request');
            return { outcome: { outcome: 'cancelled' } };
        }

        return await new Promise((resolve) => { this.pendingPermissions.set(toolCallId, { resolve }); });
    }
