...
import { killProcessByChildProcess } from '../../../utils/protocol';
...
export class AcpStdioTransport {
    ...
    constructor(options: {
        ...
        this.process.on('error', (error) => {
            log.debug(`[ACP] Process error: ${error}`);
    ...
    private handleLine(line: string): void {
        ...
        try {
            ...
            if (typeof parsed !== 'object' || parsed === null) {
                log.debug(`[ACP] Ignoring non-object JSON from stdout`);
                return;
            }
            ...
            log.debug(`[ACP] Failed to parse JSON-RPC line: ${line}`);
        ...
        if (message && 'method' in message) {
            if ('id' in message && message.id !== undefined) {
                this.handleIncomingRequest(message as JsonRpcRequest).catch((error) => {
                    log.debug(`[ACP] Error handling request: ${error}`);
                });
                return;
            }
    ...
    private async handleIncomingRequest(request: JsonRpcRequest): Promise<void> {
        ...
        try {
            ...
            this.writePayload({ jsonrpc: '2.0', id: request.id, result } satisfies JsonRpcResponse);
        } catch (error) {
            this.writePayload({
                jsonrpc: '2.0',
                id: request.id,
    ...
    private handleResponse(response: JsonRpcResponse): void {
        if (response.id === null || response.id === undefined) {
            log.debug('[ACP] Received response without id');
            return;
        }
        const pending = this.pending.get(response.id);
        if (!pending) {
            log.debug(`[ACP] Received response with no pending request: ${response.id}`);
            return;
        }
    ...
    private parseStderrError(text: string): void {
        if (!this.stderrErrorHandler) return;
        const lowerText = text.toLowerCase();

        if (lowerText.includes('status 429') || lowerText.includes('ratelimitexceeded') || lowerText.includes('rate limit')) {
            this.stderrErrorHandler({ type: 'rate_limit', message: 'Rate limit exceeded. Please wait before sending more requests.', raw: text });
            return;
        }
        if (lowerText.includes('status 404') || lowerText.includes('model not found') || lowerText.includes('not_found')) {
            this.stderrErrorHandler({ type: 'model_not_found', message: 'Model not found. Please check the model name and try again.', raw: text });
            return;
        }
        if (lowerText.includes('status 401') || lowerText.includes('status 403') || lowerText.includes('unauthenticated') || lowerText.includes('permission denied') || lowerText.includes('authentication')) {
            this.stderrErrorHandler({ type: 'authentication', message: 'Authentication failed. Please check your credentials.', raw: text });
            return;
        }
        if (lowerText.includes('quota') || lowerText.includes('resource exhausted') || lowerText.includes('resourceexhausted')) {
            this.stderrErrorHandler({ type: 'quota_exceeded', message: 'API quota exceeded. Please check your billing or wait for quota reset.', raw: text });
            return;
        }
        if (lowerText.includes('error') || lowerText.includes('failed') || lowerText.includes('exception')) {
            this.stderrErrorHandler({ type: 'unknown', message: text, raw: text });
        }
    }
}
